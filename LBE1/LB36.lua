function LB36()
    ...
    gg.addListItems(RR1)
    RR2 = {
        {["address"] = RR[1].address -100,["flags"] = 4,["value"] = "16260"},
        {["address"] = RR[1].address -88,["flags"] = 4,["value"] = "15872"},
    ...
    while true do
        GG = gg.getListItems()
        for i = 1,#GG do
            if GG[i] ["name"] == "map_info" and GG[i] ["value"] ~= "16290" then
                if GG[i] ["name"] == "map_info" and GG[i] ["value"] == "16260" then
                else 
                    gg.setValues(RR2)
                end
            end
