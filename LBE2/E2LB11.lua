function E2LB1()
    ...
    gg.addListItems(RR1)
    RR2 = {
        {["address"] = RR[1].address -100,["flags"] = 4,["value"] = "22860"},
        {["address"] = RR[1].address -88,["flags"] = 4,["value"] = "28160"},
    ...
    while true do
        GG = gg.getListItems()
        for i = 1,#GG do
            if GG[i] ["name"] == "map_info" and GG[i] ["value"] ~= "22890" then
                if GG[i] ["name"] == "map_info" and GG[i] ["value"] == "22860" then
                else 
                    gg.setValues(RR2)
                end
            end
