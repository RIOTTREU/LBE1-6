function E2LB1()
    ...
    gg.addListItems(RR1)
    RR2 = {
        {["address"] = RR[1].address -100,["flags"] = 4,["value"] = "28660"},
        {["address"] = RR[1].address -88,["flags"] = 4,["value"] = "17920"},
        {["address"] = RR[1].address -84,["flags"] = 4,["value"] = "12800"},
        {["address"] = RR[1].address + 204,["flags"] = 4,["value"] = "99999999"},
    }
    while true do
        GG = gg.getListItems()
        for i = 1,#GG do
            if GG[i] ["name"] == "map_info" and GG[i] ["value"] ~= "28690" then
                if GG[i] ["name"] == "map_info" and GG[i] ["value"] == "28660" then
                else 
                    gg.setValues(RR2)
                end
            end
